...
from core.sim.locations import initialize_dragon_locations, get_location_name
...
from ui.locations_window import LocationsWindow
...
class DragonGenApp(ctk.CTk):
    def __init__(self, starting_tribe="mixed"):
        ...
        self.starting_tribe = starting_tribe

        self.title("DragonGen")
        self.geometry("1100x750")

        # Game state
        self.selected_tribe = self.starting_tribe.capitalize()
        self.world = generate_starting_world(self.starting_tribe)
    ...
    def create_header(self):
        ...
        self.tribe_selector.set(self.selected_tribe)
    ...
    def create_controls(self):
        ...
        self.locations_button = ctk.CTkButton(
            self.control_frame,
            text="Locations",
            command=self.open_locations_window
        )
        self.locations_button.pack(side="left", padx=10, pady=10)

    def open_locations_window(self):
        LocationsWindow(self, self.world)
    ...
    def get_notable_relationships_text(self, d):
        bonded = []
        conflicts = []
        broken = []
        ...
        for memory in d.memory_flags:
            ...
            if flag == "bonded_with":
                bonded.append(other.name)
            elif flag == "major_conflict":
                conflicts.append(other.name)
            elif flag == "relationship_broken":
                broken.append(other.name)

        lines = []

        if bonded:
            lines.append("Bonded With:")
            for name in sorted(set(bonded)):
                lines.append(f"- {name}")

        if conflicts:
            if lines:
                lines.append("")
            lines.append("Major Conflicts:")
        ...
        if broken:
            if lines:
                lines.append("")
            lines.append("Broken Relationships:")
    ...
    def get_social_perception_text(self, d):
        living = [x for x in self.world.dragons if x.status == "Alive" and x.id != d.id]

        rows = []

        for other in living:
            trust = d.trust.get(other.id, 0)
            resentment = d.resentment.get(other.id, 0)
            perception = d.perceived_reputation.get(other.id, 0)

            score = trust - (resentment * 1.25) + (perception * 0.75)

            if abs(score) >= 0.5 or trust > 0 or resentment > 0 or abs(perception) > 0:
                rows.append((other.name, trust, resentment, perception, score))
        ...
        trusted = [r for r in rows if r[4] > 0][:3]
        distrusted = [r for r in rows if r[4] < 0]
        distrusted = sorted(distrusted, key=lambda x: x[4])[:3]

        text = "Trusted / Positive View:\n"
        for name, trust, resentment, perception, score in trusted:
            label = self.interpret_relationship(score)

            text += (
                f"- {name}: {label} "
                f"(T:{trust:.1f} R:{resentment:.1f} P:{perception:.1f})\n"
            )

        text += "\nDistrusted / Negative View:\n"
        for name, trust, resentment, perception, score in distrusted:
            label = self.interpret_relationship(score)
    ...
    def interpret_relationship(self, score):
        if score >= 3:
            return "Strong bond"
        elif score >= 1.5:
            return "Positive"
    ...
    def refresh_details(self):
        ...
        social_perception_text = self.get_social_perception_text(d)
        notable_relationships_text = self.get_notable_relationships_text(d)
        ...
        self.detail_text.insert(
            ...
            f"Social Perception:\n{social_perception_text}\n\n"
            f"Notable Relationships:\n{notable_relationships_text}\n\n"
