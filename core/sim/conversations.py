...
def get_conversation_context(world, a, b):
    ...
    same_location = getattr(a, "location", None) == getattr(b, "location", None)
    location = getattr(a, "location", None) if same_location else None

    return {
        "trust_ab": trust_ab,
        "trust_ba": trust_ba,
        "resentment_ab": resentment_ab,
        "resentment_ba": resentment_ba,
        "are_friends": b.id in a.friends or a.id in b.friends,
        "are_rivals": b.id in a.rivals or a.id in b.rivals,
        "a_injured": bool(getattr(a, "injuries", [])),
        "b_injured": bool(getattr(b, "injuries", [])),
        "world_tension": getattr(world, "tension", 0),
        "same_location": same_location,
        "location": location,
    }

def get_context_narration(world, a, b):
    ctx = get_conversation_context(world, a, b)

    if ctx["are_rivals"]:
        return f"There was history between {a.name} and {b.name}, and neither dragon seemed able to forget it."

    if ctx["resentment_ab"] >= 4 or ctx["resentment_ba"] >= 4:
        return f"The conversation carried old resentment before either dragon even spoke."

    if ctx["are_friends"] and (ctx["trust_ab"] >= 3 or ctx["trust_ba"] >= 3):
        return f"There was an ease between them that only came from earned trust."

    if ctx["a_injured"] or ctx["b_injured"]:
        return f"Injury made the moment quieter, forcing both dragons to choose their words carefully."

    if ctx["world_tension"] >= 3:
        return f"The tribe's unease pressed around them, making even ordinary words feel heavier."

    if ctx["location"] == "training_grounds":
        return f"The smell of dust and effort still clung to the training grounds around them."

    if ctx["location"] == "healer_den":
        return f"The quiet of the Healer's Den made the conversation feel more fragile."

    if ctx["location"] == "queen_palace":
        return f"Under the shadow of the Queen's Palace, every word felt more formal than usual."
...
def get_available_conversation_topics(world, a, b):

    topics = [
        {"id": "auto", "text": "Let the conversation unfold naturally"},
        {"id": "neutral", "text": "Personal conversation"},
    ]
    ...
    if resentment_ab >= 4 or resentment_ba >= 4:
        topics.append({"id": "repair", "text": "Repair tension"})

    if a.perceived_reputation or b.perceived_reputation:
        topics.append({"id": "rumors", "text": "Discuss rumors"})

    if getattr(world, "tension", 0) >= 3:
        topics.append({"id": "leadership", "text": "Discuss leadership"})

    if getattr(world, "tribal_relations", None):
        topics.append({"id": "tribal_relations", "text": "Discuss tribal relations"})

    if (
        getattr(a, "location", None) == "training_grounds"
        and getattr(b, "location", None) == "training_grounds"
    ):
        topics.append({"id": "sparring", "text": "Talk after sparring"})

    if (
        getattr(a, "location", None) == "healer_den"
        and getattr(b, "location", None) == "healer_den"
    ):
        topics.append({"id": "healing_talk", "text": "Talk quietly in the Healer's Den"})

    if (
        getattr(a, "location", None) == "queen_palace"
        and getattr(b, "location", None) == "queen_palace"
    ):
        topics.append({"id": "palace_talk", "text": "Speak formally at the Queen's Palace"})
...
def get_contextual_opening_line(world, a, b, convo_type):
    ctx = get_conversation_context(world, a, b)

    if convo_type == "friendly":
        if ctx["are_rivals"]:
            return f'"We used to get along better than this," {a.name} said carefully.'

        if ctx["trust_ab"] >= 4 or ctx["trust_ba"] >= 4:
            return f'"I trust you more than most dragons here," {a.name} said.'

        if ctx["world_tension"] >= 3:
            return f'"Even talking calmly feels harder lately," {a.name} admitted.'

        if ctx["a_injured"] or ctx["b_injured"]:
            return f'"You should not be carrying all of this alone," {a.name} said quietly.'

    if convo_type == "tense":
        if ctx["resentment_ab"] >= 5 or ctx["resentment_ba"] >= 5:
            return f'"This has been poisoning things between us for too long," {a.name} said.'

        if ctx["are_rivals"]:
            return f'"I know we are not exactly friends, but this still needs to be said," {a.name} said.'

        if ctx["world_tension"] >= 3:
            return f'"The whole tribe feels ready to snap, and this is not helping," {a.name} said.'

    if convo_type == "neutral":
        if ctx["same_location"] and ctx["location"] == "training_grounds":
            return f'"Hard to think clearly after training like that," {a.name} said.'

        if ctx["same_location"] and ctx["location"] == "healer_den":
            return f'"This place makes every conversation feel quieter," {a.name} said.'

        if ctx["world_tension"] >= 3:
            return f'"Everyone seems tense lately," {a.name} said.'

    return None


def build_conversation(world, a, b, topic="auto"):
    if topic == "auto":
        convo_type = get_conversation_type(a, b)
    else:
        convo_type = topic

    if convo_type == "palace_talk":
        text = (
            f"{a.name} and {b.name} stood beneath the authority of the Queen's Palace.\n\n"
            f"The conversation felt heavier here, where words carried consequence.\n\n"
            f'"We should speak carefully," {a.name} said.'
        )
        ...
        return {
            ...
            "text": text,
            "options": [
                {"id": "appeal_to_loyalty", "text": f"{a.name} appeals to loyalty and duty"},
                {"id": "seek_judgment", "text": f"{a.name} asks for the matter to be judged fairly"},
                {"id": "use_pressure", "text": f"{a.name} applies political pressure"},
            ],
        }

    if convo_type == "healing_talk":
        text = (
            f"{a.name} and {b.name} sat quietly in the Healer's Den.\n\n"
            f"The air was calm, filled with the scent of herbs.\n\n"
            f'"You seem troubled," {a.name} said softly.'
        )
        ...
        return {
            ...
            "text": text,
            "options": [
                {"id": "offer_support", "text": f"{a.name} offers genuine support"},
                {"id": "give_advice", "text": f"{a.name} gives practical advice"},
                {"id": "dismiss_feelings", "text": f"{a.name} dismisses their concerns"},
            ],
        }

        
    if convo_type == "sparring":
        text = (
            f"{a.name} and {b.name} stood at the training grounds after a hard sparring session.\n\n"
            f'"You fight harder than I expected," {a.name} said.\n\n'
            f"{b.name} watched them carefully, still catching their breath."
        )
        ...
        return {
            ...
            "text": text,
            "options": [
                {"id": "show_respect", "text": f"{a.name} shows respect for {b.name}'s strength"},
                {"id": "challenge_more", "text": f"{a.name} challenges {b.name} to push harder"},
                {"id": "mock_weakness", "text": f"{a.name} mocks {b.name}'s weakness"},
            ],
        }


    if convo_type == "rumors":
        text = (
            f'{a.name} lowered their voice.\n\n'
            f'"There has been talk around the tribe," {a.name} said.\n\n'
            f'{b.name} glanced aside, as if weighing what they had heard.'
        )
        ...
        return {
            ...
            "text": text,
            "options": [
                {"id": "ask_rumor", "text": f"{a.name} asks what {b.name} has heard"},
                {"id": "challenge_rumor", "text": f"{a.name} challenges the rumors directly"},
                {"id": "spread_concern", "text": f"{a.name} warns {b.name} that something feels wrong"},
            ],
        }
    ...
    if memory_override == "abandonment":
        ...
        contextual_line = get_contextual_opening_line(world, a, b, convo_type)

        if contextual_line:
            line1 = contextual_line
        else:
    ...
    context_line = get_context_narration(world, a, b)
...
def get_player_response_line(a, b, convo_type, option_id):
    ...
    ctx = get_conversation_context(world=None, a=a, b=b)
    ...
    lines = {
        "friendly": {
            "open_up": {
                ...
                "Neutral": [
                    f'"I’m glad we’re talking honestly," {a.name} said.' if ctx["trust_ab"] < 3 else
                    f'"I trust you enough to say this properly," {a.name} said.',

                    f'"I’d rather say it clearly than leave it unspoken," {a.name} said.' if ctx["world_tension"] < 3 else
                    f'"Things feel too unstable to leave this unsaid," {a.name} said.'
                ],
            },
            "stay_guarded": {
                "Kind": [
...
def apply_conversation_choice(world, a, b, convo_type, option_id):
    ...
    a_view_of_b = a.perceived_reputation.get(b.id, 0)
    b_view_of_a = b.perceived_reputation.get(a.id, 0)

    if convo_type == "palace_talk":
        if option_id == "appeal_to_loyalty":
            a.trust[b.id] = a.trust.get(b.id, 0) + 0.5
            b.trust[a.id] = b.trust.get(a.id, 0) + 0.5
            world.tension = max(0, world.tension - 0.04)

            return (
                f'"We owe the tribe better than suspicion," {a.name} said.',
                f'"Maybe we do," {b.name} replied.',
                "The appeal to loyalty softened the exchange."
            )

        elif option_id == "seek_judgment":
            a.resentment[b.id] = max(0, a.resentment.get(b.id, 0) - 0.3)
            b.resentment[a.id] = max(0, b.resentment.get(a.id, 0) - 0.3)
            world.tension = max(0, world.tension - 0.02)

            return (
                f'"Then let it be judged fairly," {a.name} said.',
                f'"Fairly, then," {b.name} answered.',
                "The matter felt contained, even if not fully resolved."
            )

        elif option_id == "use_pressure":
            a.trust[b.id] = max(0, a.trust.get(b.id, 0) - 0.2)
            b.resentment[a.id] = b.resentment.get(a.id, 0) + 0.8
            world.tension += 0.06

            return (
                f'"You know how this will look if others hear of it," {a.name} said.',
                f'{b.name} narrowed their eyes. "So that is what this is."',
                "The palace made the pressure sharper, not softer."
            )

    if convo_type == "healing_talk":
        if option_id == "offer_support":
            a.trust[b.id] = a.trust.get(b.id, 0) + 1.0
            b.trust[a.id] = b.trust.get(a.id, 0) + 1.0
            b.resentment[a.id] = max(0, b.resentment.get(a.id, 0) - 0.5)
            world.tension = max(0, world.tension - 0.05)

            return (
                f'"I’m here for you," {a.name} said quietly.',
                f'{b.name} let out a slow breath. "That… helps more than you know."',
                "Trust deepened between them."
            )

        elif option_id == "give_advice":
            a.trust[b.id] = a.trust.get(b.id, 0) + 0.5
            b.resentment[a.id] = max(0, b.resentment.get(a.id, 0) - 0.2)

            return (
                f'"You need to think clearly about this," {a.name} said.',
                f'"Maybe… you’re right," {b.name} admitted.',
                "The conversation brought some clarity, though not warmth."
            )

        elif option_id == "dismiss_feelings":
            b.resentment[a.id] = b.resentment.get(a.id, 0) + 0.7
            world.tension += 0.04

            return (
                f'"It’s not that serious," {a.name} said dismissively.',
                f'{b.name} looked away. "Right… of course."',
                "The moment of vulnerability was shut down."
            )


    if convo_type == "sparring":
        if option_id == "show_respect":
            a.trust[b.id] = a.trust.get(b.id, 0) + 0.8
            b.trust[a.id] = b.trust.get(a.id, 0) + 0.6
            world.tension = max(0, world.tension - 0.03)

            return (
                f'"You handled yourself well," {a.name} said.',
                f'"So did you," {b.name} replied.',
                "Respect grew between them after the sparring match."
            )

        elif option_id == "challenge_more":
            a.trust[b.id] = a.trust.get(b.id, 0) + 0.3
            b.resentment[a.id] = b.resentment.get(a.id, 0) + 0.3
            world.tension += 0.03

            return (
                f'"You could push harder than that," {a.name} said.',
                f'"Careful," {b.name} replied. "Not every challenge is wisdom."',
                "The exchange left both respect and irritation behind."
            )

        elif option_id == "mock_weakness":
            a.resentment[b.id] = a.resentment.get(b.id, 0) + 0.5
            b.resentment[a.id] = b.resentment.get(a.id, 0) + 1.0
            world.tension += 0.06

            return (
                f'"Is that really your best?" {a.name} asked.',
                f'{b.name} went still. "Say that again when you are ready to bleed for it."',
                "The sparring session left a sharper wound than either admitted."
            )

    if convo_type == "rumors":
        from core.sim.rumors import spread_rumor

        if option_id == "ask_rumor":
            spread_rumor(world, b.id, a.id, effect=0.2)
            result_text = f"{a.name} listened carefully. The rumor quietly spread."

        elif option_id == "challenge_rumor":
            # reduces belief
            a.perceived_reputation[b.id] = a.perceived_reputation.get(b.id, 0) - 1
            result_text = f"{a.name} pushed back against the rumors, casting doubt."

        elif option_id == "spread_concern":
            spread_rumor(world, a.id, b.id, effect=0.3)
            world.tension += 0.2
            result_text = f"{a.name}'s concern added to the growing unease in the tribe."

        else:
            result_text = "The rumor conversation faded without impact."

        log_event(
            world,
            result_text,
            involved_ids=[a.id, b.id],
            event_type="rumor_conversation",
            importance=3
        )

        return (
            f"{a.name} lowered their voice.",
            f"{b.name} reacted cautiously.",
            result_text
        )
    ...
    if convo_type == "repair":
        ...
        if option_id == "apologize":
            ...
            if a.id in b.resentment:
                b.resentment[a.id] = max(0, b.resentment[a.id] - reduction_b)

            if b.id in a.resentment:
                a.resentment[b.id] = max(0, a.resentment[b.id] - reduction_a)
            ...
            # bad perception makes accusations land harder
            if a_view_of_b < -1:
                increase_a += 1
            ...
            # good perception softens accusations
            if a_view_of_b > 2:
                increase_a -= 1
            ...
            a.resentment[b.id] = a.resentment.get(b.id, 0) + increase_a
            b.resentment[a.id] = b.resentment.get(a.id, 0) + increase_b
    ...
    if convo_type == "friendly":
        if option_id == "open_up":
            ...
            # Trust makes openness more effective
            if ctx["trust_ab"] >= 3:
                base_gain += 1

            # High resentment reduces effectiveness
            if ctx["resentment_ab"] >= 3:
                base_gain -= 1

            # High world tension makes openness riskier
            if ctx["world_tension"] >= 3:
                base_gain -= 1

            # Injured dragons respond more strongly (positive or negative)
            if ctx["a_injured"] or ctx["b_injured"]:
                base_gain += 1

            # Clamp so it doesn’t go crazy
            base_gain = max(0, base_gain)

            a_gain = base_gain + a_mod["open_up_bonus"] + a_mod["trust_bonus"]
            b_gain = base_gain + b_mod["trust_bonus"]
