...
from core.sim.choice_generation.conversation import create_ai_conversation_choice
...
def add_rival_event(world, a, b):
    ...
    # subtle social support hint
    supporters_a = [
        d for d in world.dragons
        if d.status == "Alive"
        and d.id not in {a.id, b.id}
        and d.perceived_reputation.get(a.id, 0) > 1
    ]
    ...
    supporters_b = [
        ...
        if d.status == "Alive"
        and d.id not in {a.id, b.id}
        and d.perceived_reputation.get(b.id, 0) > 1
    ]

    if supporters_a and random.random() < 0.25:
        support_texts = [
            f" A few dragons seemed quietly sympathetic to {a.name}.",
            f" Not everyone agreed with what was happening to {a.name}.",
            f" Some nearby dragons appeared to take {a.name}'s side.",
        ]

        text += random.choice(support_texts)

    if supporters_b and random.random() < 0.25:
        support_texts = [
            f" A few dragons seemed quietly sympathetic to {a.name}.",
            f" Not everyone agreed with what was happening to {a.name}.",
            f" Some nearby dragons appeared to take {a.name}'s side.",
    ...
    # rival conflicts create rumors
    spread_rumor(world, a.id, b.id, effect=-0.2)
    ...
    if level == 3:
        ...
        if outcome == "injury":
            ...
            spread_rumor(world, victim.id, a.id, effect=-0.3)
            spread_rumor(world, victim.id, b.id, effect=-0.3)
